...
export default function AuthScreen() {
  ...
  const [step, setStep] = useState('welcome'); // 'welcome', 'signup', 'login', 'otp'
  ...
  const handleAuth = async (e) => {
    ...
    const endpoint = authType === 'signup' ? '/user/sign-up' : '/user/log-in';

    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_FRONT_URL}${endpoint}`, {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        setError(data.message || `Failed to ${authType === 'signup' ? 'sign up' : 'send OTP'}`);
      }
    } catch (err) {
            ...
            <div className="bg-white rounded-2xl shadow-xl p-8 border border-gray-100">
              ...
              <div className="space-y-4">
                ...
                <button
                  ...
                  className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-semibold py-4 rounded-lg transition-all shadow-lg shadow-emerald-600/20 hover:shadow-xl"
                >
                  Create New Account
                </button>
                ...
                <button
                  ...
                  className="w-full bg-white hover:bg-gray-50 text-gray-900 font-semibold py-4 rounded-lg transition-all border-2 border-gray-200 hover:border-emerald-500"
                >
                  Sign In to Existing Account
                </button>
              </div>

              <p className="text-center text-xs text-gray-500 leading-relaxed pt-6">
      ...
      <div className="min-h-screen bg-white flex">
        ...
        <div className="hidden lg:flex lg:w-1/2 bg-gradient-to-br from-emerald-500 via-teal-500 to-emerald-600 relative overflow-hidden">
          ...
          <div className="relative z-10 flex flex-col justify-center items-center w-full p-16 text-white">
            ...
            <div className="text-center space-y-6 max-w-md">
              <h1 className="text-5xl font-bold leading-tight">
                {isSignup ? 'Join Convoo Today' : 'Welcome Back!'}
              </h1>
              <p className="text-xl text-white/90 leading-relaxed">
                {isSignup 
                  ...
                  <p className="text-sm text-gray-600">
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        setAuthType(isSignup ? 'login' : 'signup');
                        setPhone('');
                        setError('');
                      }}
    ...
    <div className="min-h-screen bg-white flex">
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-8 bg-gray-50">
        <div className="w-full max-w-md">
          ...
          <button
            onClick={() => {
              setStep(authType === 'signup' ? 'signup' : 'login');
              setOtp(['', '', '', '']);
              setError('');
            }}
            className="flex items-center gap-2 text-gray-600 hover:text-gray-900 mb-8 transition-colors"
              ...
              <div className="flex items-center justify-center gap-4">
                {otp.map((digit, index) => (
                  <input
                    ...
                    className="w-16 h-16 bg-gray-50 border-2 border-gray-200 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 text-gray-900 text-3xl text-center rounded-lg outline-none transition-all font-semibold"
                    maxLength={1}
                  />
                ))}
              </div>
